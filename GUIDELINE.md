...
|---|---|---|
| Nome da unidade de saúde | Substituída pela denominação genérica `Unidade de Saúde 1`, `Unidade de Saúde 2`, ... | Implementado em `02_anon_unidade_saude.py` para schema operacional (`tb_unidade_saude`/`tb_estabelecimento`) e DW (`tb_dim_unidade_saude.no_unidade_saude`). |
...
| Categoria e registro profissional | Categoria e registros de saúde preservados. Nome fictício com sobrenome "Teste", escolhido entre nomes representativos de diversos gêneros (evitar viés psicodemográfico). Registro profissional substituído por `99999`. | Implementado em `05_anon_profissional.py` para `tb_dim_profissional.no_profissional` e colunas operacionais prováveis de registro profissional. CNS (`nu_cns`) segue pendente por exigir regra própria, não CPF. |
| CPF do profissional de saúde | Substituído por CPF fictício, válido, gerado aleatoriamente. | Implementado em `01_anon_cpf.py` (`tb_prof.nu_cpf`, schema operacional). |
| CPF do cidadão | Substituído por CPF fictício, válido, gerado aleatoriamente. | Implementado em `01_anon_cpf.py` para schema operacional e colunas de CPF do DW inventariadas no TSV (`nu_cpf_cidadao`/`co_identificacao`). |
| Data de nascimento / idade do cidadão | O dia de nascimento substituído por número aleatório de 1 a 30/31. Mês e ano preservados. A data de cada atendimento deve ser redimensionada para preservar o número de dias entre nascimento e atendimento. | Implementado em `04_anon_datas_cidadao.py` para tabelas do DW com CPF e data de nascimento mapeadas; linhas sem CPF são preservadas. |
...
| E-mail do cidadão | Substituído pelo termo genérico `cidadao@teste.br`. | Implementado em `03_anon_email.py` para schema operacional e colunas de e-mail do DW (`tb_fat_avaliacao_elegibilidade.no_email`, `tb_fat_cad_individual.no_email`). |
| Endereço do cidadão | Substituído por outro endereço válido do mesmo município, obtido aleatoriamente entre os endereços existentes na base. | Implementado em `06_anon_endereco.py` para os campos de endereço mapeados no DW. A migration só atua quando encontra coluna de município na tabela, para não misturar municípios. |
...
| Data-hora dos registros de saúde | Sincronizada com as modificações na data de nascimento do cidadão. A temporalidade entre registros longitudinais de um mesmo cidadão deve ser preservada para manter a coerência da evolução clínica. | Implementado em `04_anon_datas_cidadao.py` para colunas de data-hora mapeadas no TSV. |
